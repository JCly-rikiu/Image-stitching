...
void WarpImagesTogether(const std::vector<cv::Mat>& image_data, PanoramaLists& panorama_lists) {
  ...
  for (int pano_index = 1; auto& list : panorama_lists) {
    ...
    for (auto [image, ti, tj] : list) {
      ...
      cv::warpAffine(image_data[image], temp, translation_mat,
                     cv::Size(image_data[image].cols + 1, rows));  // Padding for integer pixel position
...
void SaveFeatureMatches(const std::vector<cv::Mat>& image_data, const PanoramaLists& panorama_lists,
                        const FeatureMatches& match_points) {
  std::cout << "[Save feature matches...]" << std::endl;
